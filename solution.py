...
class Reader:
    """
    A Reader class to render some data
    ...
    Examples
    ========
    >>> csv_reader = Reader('orders.csv', 'barcodes.csv')
    ...
    def __init__(self, input_1:'orders.csv', input_2:'barcodes.csv',
        ...
        self.ddf_2 = df.read_csv(input_2, dtype=str)
        ...
        # unsuccessfull also I tried in Hard way and it was successfull.
        ...
        if client:
            client = Client("192.168.6.40:8786")# n_workders=2, threads_per_worker=2, momory_limit='1GB')
            print(client)
    ...
    def output(self):
        ...
        for _, row in self.ddf_1.iterrows():
            ...
            print(f"{o_id}, {customer_id}, [{', '.join(barcodes)}]")
    ...
    def validate(self):
        ...
        size_befor = len(self.ddf_2)
        self.ddf_2 = self.ddf_2.drop_duplicates(keep='first')
        droped_lines_n = size_befor - len(self.ddf_2)
        if droped_lines_n: 
    ...
    def _cmp_and_log(self, message, map_df):
        ...
        if not df.empty:
            ...
            self.LOGGER.error(df.to_string())
...
def parse_args():
    ...
    parser.add_argument('-input_order', dest='input_order', default='orders.csv',
                        help="Input order csv file path")
    parser.add_argument('-input_barcode', dest='input_barcode', default='barcodes.csv',
...
if __name__ == "__main__":
    args = parse_args() 
    csv_reader = Reader(args.input_order, args.input_barcode)
    csv_reader.validate()
    #csv_reader.output()
    csv_reader.top5()
